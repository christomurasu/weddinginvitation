        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          <thead>
            <tr style={{ background: "#faf8f4", borderBottom: "1px solid #e4ddd0" }}>
              ...
              {["Guest", "Code", "WA", "Table", "Max", "Attending", "Tipe", "Bahasa", "RSVP", "Link"].map(h => (
          ...
          <tbody>
              ...
              <tr key={guest.id} style={{ borderBottom: "1px solid #f5f0e8", background: selected.has(guest.id) ? "#fef9ec" : "transparent" }}>
                ...
                <td style={{ padding: "14px 16px" }}>
                  {guest.phone ? (
                    <a href={`https://wa.me/62${guest.phone.replace(/^0/, "").replace(/\D/g, "")}?text=${encodeURIComponent(waMessage(guest))}`}
                      ...
                      style={{ display: "inline-flex", alignItems: "center", gap: 4, background: "#25D366", color: "#fff", padding: "4px 10px", fontSize: 11, textDecoration: "none", fontWeight: 600, letterSpacing: "0.05em" }}>
                      WA ↗
                    </a>
                  ) : (
                ...
                <td style={{ padding: "14px 16px", textAlign: "center" }}>
                  {guest.rsvp === "confirmed" ? (
                    <span style={{ color: "#3b6d11", fontSize: 13, fontWeight: 500 }}>{guest.actual_attendees ?? 1}</span>
                  ) : (
                    <span style={{ color: "#b4b2a9", fontSize: 12 }}>—</span>
                  )}
                </td>
                <td style={{ padding: "14px 16px" }}>
                  <span style={{ fontSize: 10, padding: "3px 8px", background: guest.invitation_type === "ceremony" ? "#faeeda" : "#e6f1fb", color: guest.invitation_type === "ceremony" ? "#854f0b" : "#185fa5", letterSpacing: "0.08em", textTransform: "uppercase" }}>
                ...
                <td style={{ padding: "14px 16px" }}>
                  <div style={{ display: "flex", gap: 4 }}>
                    {["en", "id"].map(lang => (
                      <button
                        key={lang}
                        onClick={() => handleLanguageChange(guest.id, lang)}
                ...
                <td style={{ padding: "14px 16px" }}>
                  <span style={{ fontSize: 11, padding: "3px 10px", background: guest.rsvp === "confirmed" ? "#eaf3de" : guest.rsvp === "declined" ? "#fcebeb" : "#f5f0e8", color: guest.rsvp === "confirmed" ? "#3b6d11" : guest.rsvp === "declined" ? "#a32d2d" : "#888780" }}>
                    {guest.rsvp}
                  </span>
                </td>
                <td style={{ padding: "14px 16px" }}>
            ...
            {guests.length === 0 && (
              <tr>
                <td colSpan={11} style={{ padding: "40px", textAlign: "center", color: "#888780", fontSize: 13 }}>
