...
export default function CSVImport({ weddingId }: { weddingId: string }) {
  ...
  const COLUMNS = ["name", "greeting", "phone", "invitation_type", "max_attendees", "seats", "note"]
  ...
  function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (ev) => {
      try {
        ...
        setParseError("Gagal membaca file CSV. Pastikan format sesuai template.")
      }
    }
  ...
  async function handleImport() {
    ...
    for (const row of preview) {
      ...
      const { error } = await supabase.from("guests").insert({
        ...
        note: row.note || null,
        rsvp: "pending",
        ceremony_rsvp: "pending",
        reception_rsvp: "pending",
        ceremony_adults: 0,
        ceremony_kids: 0,
        reception_adults: 0,
        reception_kids: 0,
  ...
  const templateCSV = `name,greeting,phone,invitation_type,max_attendees,seats,note\nChristopher Sonny,Mr. Christopher,081234567890,full,2,2,\nFelyn Karina,Mrs. Felyn,089876543210,ceremony,1,1,Special guest`
  ...
  return (
    <div style={{ background: "#fff", border: "1px solid #e4ddd0", padding: "24px", marginBottom: 24 }}>
      ...
      <div style={{
        background: "#fdf8ee", border: "1px solid #e4ddd0",
        padding: "12px 14px", marginBottom: 16, fontSize: 12,
        fontFamily: "monospace", color: "#2c2c2a", lineHeight: 1.8
      }}>
        name, greeting, phone, invitation_type, max_attendees, seats, note
      </div>

      <div style={{
        background: "#f5f0e8", border: "1px solid #e4ddd0",
        padding: "12px 14px", marginBottom: 20, fontSize: 12, lineHeight: 1.8
      }}>
        <p style={{ color: "#888780", marginBottom: 4, fontSize: 10, letterSpacing: "0.1em", textTransform: "uppercase" }}>
          Keterangan
        </p>
        {[
          ["name", "✅ Wajib", "Nama lengkap tamu"],
          ["greeting", "✅ Wajib", "Sapaan (Mr., Mrs., The ... Family)"],
          ["phone", "❌ Opsional", "Nomor WA tanpa +62 (misal: 08123456789)"],
          ...
          <div key={col} style={{ display: "flex", gap: 8, marginBottom: 2 }}>
            <span style={{ fontFamily: "monospace", color: "#2c2c2a", minWidth: 120 }}>{col}</span>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          ...
          <input ref={fileRef} id="csv-upload" type="file" accept=".csv"
        ...
        {parseError && (
          <p style={{ fontSize: 12, color: "#a32d2d", marginTop: 8 }}>{parseError}</p>
        )}
      </div>

      {result && (
        <div style={{
          background: result.error > 0 ? "#fef3c7" : "#f0fdf4",
          border: `1px solid ${result.error > 0 ? "#fbbf24" : "#86efac"}`,
              ...
              <tbody>
                {preview.map((row, i) => (
                  <tr key={i} style={{ background: i % 2 === 0 ? "#fff" : "#fdf8ee" }}>
                    {COLUMNS.map(h => (
                      <td key={h} style={{
                        padding: "8px 12px", borderBottom: "1px solid #f0ebe3",
                        color: "#2c2c2a", fontSize: 12
                      }}>
      ...
      {preview.length > 0 && (
        <div style={{ display: "flex", gap: 12 }}>
          <button
            ...
            disabled={loading}
            style={{
              background: loading ? "#888780" : "#2c2c2a",
