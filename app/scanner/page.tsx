      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "28px 24px" }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              {nameResults.length > 0 && (
                <div style={{ background: "#1a1a18", border: "1px solid #444441", borderTop: "none", maxHeight: 220, overflowY: "auto" }}>
                  {nameResults.map(g => (
                    <div key={g.id} onClick={() => { setNameSearch(""); setNameResults([]); lookupGuest(g.code) }}
        ...
        {guest && !loading && (
          <div>
            <div style={{
              background: "#1a1a18",
              border: `1px solid ${confirmed ? "#3b6d11" : wrongType ? "#a32d2d" : !isConfirmedForMode ? "#a32d2d" : alreadyScannedForMode ? "#854f0b" : "#b8965a"}`,
            ...
            {!confirmed && !wrongType && isConfirmedForMode && !alreadyScannedForMode && (
              <button onClick={confirmPresence} disabled={confirming} style={{
                width: "100%", background: "#3b6d11", color: "#fff", border: "none", padding: "15px", fontSize: 12,
                letterSpacing: "0.2em", textTransform: "uppercase", cursor: confirming ? "not-allowed" : "pointer",
                opacity: confirming ? 0.7 : 1, fontFamily: "inherit", marginBottom: 10
              }}>
                {confirming ? "Confirming..." : `✓ Check-in ${mode === "ceremony" ? "Pemberkatan" : "Resepsi"} · ${adultsForMode} Dewasa${kidsForMode > 0 ? ` · ${kidsForMode} Anak` : ""}`}
              </button>
            )}
