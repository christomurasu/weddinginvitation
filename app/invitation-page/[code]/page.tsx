...
export default async function InvitationPage({
  ...
  const weddingDay = wedding.wedding_date_iso
    ? new Date(wedding.wedding_date_iso).getDate()
    : ""
  const dayName = wedding.date?.split(",")[0]?.trim() ?? ""
  const monthYear = wedding.date?.split(",")[1]?.trim().split(" ").slice(1).join(" ") ?? ""
    ...
    <>
      ...
      <div className="snap-section" style={{ background: "#2c2c2a", overflowY: "auto" }}>
        <div style={{
          ...
          <div style={{ marginTop: 24, display: "flex", flexDirection: "column", gap: 12, overflowY: "auto", flex: 1 }}>
